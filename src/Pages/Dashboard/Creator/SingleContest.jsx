...
const SingleContest = ({items, handleDelete}) => {
...
{status}
      ...
      <td className='px-5 py-5 border-b border-gray-200 bg-white text-text-base'>
        <p className='text-gray-900 whitespace-no-wrap'>
        <Link to={`/dashboard/my-listing-edit/${_id}`}>
       <button disabled={status === "approved"}   className={`px-6 py-2 rounded-full ${status === "approved" ? 'bg-gray-300' : 'bg-green-200'} ${status === "approved" ? 'disabled' : ''}`}>Edit</button>
       </Link>
        </p>
      </td>
      <td className='px-5 py-5 border-b border-gray-200 bg-white text-text-base'>
